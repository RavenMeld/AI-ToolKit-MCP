...
To also require authentication on `GET /tools`:

```bash
export MCP_HTTP_AUTH_TOKEN=change-me
export MCP_HTTP_AUTH_FOR_TOOLS=true
python mcp_http_server.py --host 127.0.0.1 --port 8080
```
...
export MCP_HTTP_REQUIRE_TRUSTED_PROXY_CIDRS=false
python mcp_http_server.py --host 127.0.0.1 --port 8080
```
...
- `MCP_HTTP_REQUIRE_TRUSTED_PROXY_CIDRS` (default: `false`, requires non-empty trusted proxy CIDRs when proxy trust is enabled)
...
- `--trusted-proxy-cidrs`
- `--require-trusted-proxy-cidrs` / `--allow-any-proxy-source`
...
- proxy-aware client identity (optional, disabled by default):
  ...
  - optional strict mode:
    - `MCP_HTTP_REQUIRE_TRUSTED_PROXY_CIDRS=true` or `--require-trusted-proxy-cidrs`
    - startup fails if proxy trust is enabled without a non-empty trusted proxy CIDR allowlist
